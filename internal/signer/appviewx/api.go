...
func (signer *ApViewXSigner) DownloadCertificate(ctx context.Context, appviewxEnv *AppViewXEnv, sessionID, resourceID string, retryTimes int) (certificateContents string, err error) {
	...
	url, err := common.GenerateURL(ctx, appviewxEnv.AppViewXIsHTTPS, appviewxEnv.AppViewXHost, appviewxEnv.AppViewXPort, "/avxapi/", "certificate/download/content", queryParam)
	if err != nil {
...
func (signer *ApViewXSigner) CreateCertificate(ctx context.Context, appviewxEnv *AppViewXEnv, sessionID string, csrContent *string, isSync bool,
	...
	url, err := common.GenerateURL(ctx, appviewxEnv.AppViewXIsHTTPS, appviewxEnv.AppViewXHost, appviewxEnv.AppViewXPort, "/avxapi/", "certificate/create", queryParamMap)
	if err != nil {
		log.V(1).Info("createCertificate - Error in generating url for session ID : ", "zap.Error(err) : ", zap.Error(err))
